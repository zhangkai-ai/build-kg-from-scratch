...
def supervise(
    ...
    # add by dai
    NOW_STAY = frozenset(["供应商", "供货商", "二级供应商", "一供", "二供", "供应体系"])
    EVER_STAY = frozenset(["子公司", "持有", "有限合伙人", "普通合伙人"])
    ...
    # add by dai
    if len(NOW_STAY.intersection(intermediate_lemmas)) > 0:
        yield transaction._replace(label=1, type='pos:A成为B的供应商')

    if len(EVER_STAY.intersection(intermediate_lemmas)) > 0:
        yield transaction._replace(label=-1, type='neg:A的子公司B')
