...
# import sys
import re
import template_match.reg_util  as reg_util
# reload(sys)
# sys.setdefaultencoding('utf-8')

zh_number = u"零|一|二|三|四|五|六|七|八|九|十|百|千|万"
zh_number_dict = {u"一":"1",u"二":"2",u"三":"3",u"四":"4",u"五":"5",u"六":"6",u"七":"7",u"八":"8",u"九":"9",u"十":"10",u"百":"100",u"千":"1000",u"万":"10000"}

def deal_height(sent):
    sent = sent.replace(u'吗','')
    pattern_0 = u"(\\d|{zh_number})(尺|丈)".format(zh_number= zh_number)
    pattern_1 = u"(\\d+|{zh_number})米((\\d+|{zh_number})+)".format(zh_number = zh_number)
    pattern_2 = u"(\\d+)(厘米|公分|cm|CM)"
    pattern_3 = u"((\\d+)|{zh_number}+)$".format(zh_number = zh_number)
...
def deal_weight(sent):
    sent = sent.strip()
    pattern_1 = u'((\\d|{zh_number})+)(千克|公斤|kg)'.format(zh_number=zh_number)
    pattern_2 = u'((\\d|{zh_number})+)斤((\\d|{zh_number})+)?两?'.format(zh_number=zh_number)
    pattern_3 = u'((\\d|{zh_number})+)$'.format(zh_number=zh_number)
    sent = sent.replace(u'吗',"")
    if re.search(pattern_3, sent):
        sent = sent + u"斤"
